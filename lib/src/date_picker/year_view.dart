    ...
    if (widget.cellBuilder != null && _children.isEmpty) {
      ...
      for (int j = 0; j < viewCount; j++) {
        ...
        final double viewStartPosition = isVerticalMultiView
            ? 0
            : (currentViewIndex * width) + (currentViewIndex * widget.multiViewSpacing);
        ...
        for (int i = 0; i < visibleDatesCount; i++) {
          ...
          if ((widget.enableMultiView || widget.isHijri) &&
              DateRangePickerHelper.isLeadingCellDate(
                  currentIndex, viewStartIndex, widget.visibleDates, widget.view)) {
            xPosition += cellWidth;
            continue;
          }
...
class _ExtendableRangeSelectionRenderWidget extends MultiChildRenderObjectWidget {
  ...
  void updateRenderObject(
      BuildContext context, _ExtendableRangeSelectionRenderObject renderObject) {
    renderObject
      ..visibleDates = visibleDates
      ..cellStyle = cellStyle
      ..minDate = minDate
...
abstract class _IYearViewRenderObject extends RenderBox
  ...
  final TextPainter _textPainter = TextPainter(
      textDirection: TextDirection.ltr, maxLines: 2, textWidthBasis: TextWidthBasis.longestLine);
  ...
  List<int> _getRangeIndex(dynamic startDate, dynamic endDate, DateRangePickerView pickerView,
      int viewStartIndex, int viewEndIndex) {
    int startIndex = -1;
    int endIndex = -1;
    ...
    if (endDate != null) {
      endRangeDate =
          DateRangePickerHelper.getDate(endDate.year, endDate.month, endDate.day, _isHijri);
    }
    ...
    final dynamic viewEndDate =
        DateRangePickerHelper.getLastDate(visibleDates[viewEndIndex], pickerView, _isHijri);
    if (startDate != null) {
  ...
  String getCellSemanticsText(dynamic date) {
    if (_view == DateRangePickerView.year) {
      if (isHijri) {
        return DateRangePickerHelper.getHijriMonthText(date, localizations, 'MMMM') +
            date.year.toString();
      } else {
  ...
  void _addMouseHovering(
    ...
    if (xPosition <= _mouseHoverPosition.value!.offset!.dx &&
      ...
      canvas.drawRRect(
          RRect.fromRectAndRadius(rect, Radius.circular(cornerRadius)), _todayHighlightPaint);
    }
  }
  ...
  void _addRangeHoverEffect(
    ...
    final List<bool> hoveringDetails =
        rangeSelectionYearView._getSelectedRangePosition(currentIndex, hoveringIndex);
  ...
  void _drawYearDecoration(Canvas canvas, Decoration yearDecoration, double xPosition,
      double yPosition, double decorationPadding, double cellWidth, double cellHeight) {
    ...
    boxPainter.paint(
        ...
        ImageConfiguration(
            size: Size(cellWidth - (2 * decorationPadding), cellHeight - (2 * decorationPadding))));
  }

  TextStyle _updateCellTextStyle(int j, bool isCurrentDate, bool isSelected, bool isEnableDate,
...
class _SingleSelectionRenderObject extends _IYearViewRenderObject {
  ...
  void drawSelection(
    ...
    final double cornerRadius =
        selectionShape == DateRangePickerSelectionShape.circle ? rect.height / 2 : 3;
...
bool _isCurrentViewDateCell(
    dynamic date, int index, List<dynamic> visibleDates, bool enableMultiView, dynamic view) {
...
void _drawYearCells(PaintingContext context, Size size, _IYearViewRenderObject yearView) {
  ...
  final bool isHorizontalMultiView = yearView.enableMultiView &&
      yearView.navigationDirection == DateRangePickerNavigationDirection.horizontal;
  final bool isVerticalMultiView = yearView.enableMultiView &&
      yearView.navigationDirection == DateRangePickerNavigationDirection.vertical;
  if (isHorizontalMultiView) {
  ...
  if (isNeedWidgetPaint) {
    ...
    for (int j = 0; j < count; j++) {
      ...
      for (int i = 0; i < visibleDatesCount; i++) {
        ...
        final bool isEnableDate = DateRangePickerHelper.isBetweenMinMaxDateCell(date,
            yearView.minDate, yearView.maxDate, yearView.enablePastDates, view, yearView.isHijri);
        ...
        if (!isDisabledDate &&
            ...
            DateRangePickerHelper.isDisableDirectionDate(yearView.selectedRange, date,
                yearView.extendableRangeSelectionDirection, view, yearView.isHijri,
        ...
        if (isSelected && isEnableDate && !isDisabledDate) {
          yearView.drawCustomCellSelection(
              canvas,
              Rect.fromLTRB(xPosition, yPosition, xPosition + cellWidth, yPosition + cellHeight),
              currentIndex);
        }
  ...
  for (int j = 0; j < count; j++) {
    ...
    for (int i = 0; i < visibleDatesCount; i++) {
      ...
      final bool isActiveDate =
          _isCurrentViewDateCell(date, j, yearView.visibleDates, yearView.enableMultiView, view);
      bool isDisabledDate = DateRangePickerHelper.isDateWithInVisibleDates(
          yearView.visibleDates, yearView.disableDatesCollection, date);
      if (!isDisabledDate &&
          yearView is _ExtendableRangeSelectionRenderObject &&
      ...
      final TextStyle style = yearView._updateCellTextStyle(
          j, isCurrentDate, isSelected, isEnableDate, isActiveDate, isDisabledDate);
      ...
      if (isSelected && isEnableDate && !isDisabledDate) {
        yearView.drawSelection(canvas, cellWidth, currentIndex, highlightPadding, selectionPadding,
            textHalfHeight, centerYPosition, xPosition, yPosition, yearText);
        ...
        yearView._drawTodayHighlight(canvas, cellWidth, cellHeight, centerYPosition,
            highlightPadding, selectionPadding, textHalfHeight, xPosition, yPosition);
      }
